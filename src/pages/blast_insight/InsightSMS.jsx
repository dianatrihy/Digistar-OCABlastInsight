  ...
  useEffect(() => {
    const getBlasts = async () => {
      try {
        ...
        const aggregatedData = data.data.reduce((acc, activity) => {
          const date = new Date(activity.schedule_start).toLocaleDateString(
            'en-US',
          ...
          acc[date] += activity.messages_status.sent || 0;
          return acc;
        }, {});

        const totalSent = data.data.reduce(
          (acc, activity) => acc + (activity.messages_status.sent || 0),
          0
        );
        const totalDelivered = data.data.reduce(
          (acc, activity) => acc + (activity.messages_status.delivered || 0),
          0
        );
        const totalFailed = data.data.reduce(
          (acc, activity) => acc + (activity.messages_status.failed || 0),
          0
        );

        setTotalMessagesSent(totalSent);
        setTotalMessagesDelivered(totalDelivered);
        setTotalMessagesFailed(totalFailed);
      ...
      <div className="container">
        ...
        <div className="metrics">
          <div className="metric">
            <div className="value">
              {totalMessagesSent + totalMessagesDelivered + totalMessagesFailed}
            </div>
            <div className="label">Total Messages</div>
          </div>
          <div className="metric">
            <div className="value">{totalMessagesSent}</div>
            <div className="label">Messages Sent</div>
          </div>
          <div className="metric">
            <div className="value">{totalMessagesDelivered}</div>
            <div className="label">Messages Delivered</div>
          </div>
          <div className="metric">
            <div className="value">{totalMessagesFailed}</div>
        ...
        <div className="stats">
          ...
          <div className="stat">
            <h2>Time</h2>
            <div className="chart-container">
              <div id="time-chart">{/* Time chart implementation */}</div>
            </div>
            <div className="legend">
              <div className="item">
                <div
                  className="color"
                  style={{ backgroundColor: '#21b1f7' }}
                ></div>
              ...
              {recentBroadcastActivities.length > 0 ? (
                recentBroadcastActivities.map((activity, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      {moment(activity.schedule_start).format('DD MMMM YYYY')}
                    </td>
